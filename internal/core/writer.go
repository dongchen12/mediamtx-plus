...
func (fw *FLVWriter) apiReaderDescribe() pathAPISourceOrReader {
	return pathAPISourceOrReader{Type: "", ID: "flvWriter:" + fw.Uid}
}
...
func (fw *FLVWriter) pullStream(st *stream) error {
	var videoFormatH264 *formats.H264
	videoMedia := st.medias().FindFormat(&videoFormatH264)

	// 先暂时实现一个H264的方案, 之后需要添加H265, Opus等
	if videoFormatH264 != nil {
		st.readerAdd(fw, videoMedia, videoFormatH264, func(unit formatprocessor.Unit) {
...
func NewFLVWriter(app, title, url string, ctx http.ResponseWriter) *FLVWriter {
	...
	if _, err := ret.ctx.Write([]byte{0x46, 0x4c, 0x56, 0x01, 0x05, 0x00, 0x00, 0x00, 0x09}); err != nil { // 写flv头, 开始传输
		log.Errorf("Error on response writer")
		ret.closed = true
	}
	pio.PutI32BE(ret.buf[:4], 0) // 把header前4个字节置0, 用于后面写入数据长度, 注意传输过程中的大小端转换
...
func (flvWriter *FLVWriter) Write(p *rtp.Packet) (err error) {
	err = nil
	if flvWriter.closed {
